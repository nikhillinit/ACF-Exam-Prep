...
class ProblemAnalyzer {
  ...
  async analyzeProblem(problemText, options = {}) {
    ...
    if (!this.cache.keywords) {
      this.cache.keywords = await dataLoader.loadKeywordMappings();
      this.cache.archetypes = await dataLoader.getAllArchetypes();
      this.cache.deviations = await dataLoader.loadDeviationRegistry();
    }
    ...
    const deviationDetection = includeDeviations
      ? await this.detectDeviations(problemText, archetypeDetection.primary)
      : null;

    // Step 4: Generate solution approach
    ...
    const similarExamples = includeExamples
      ? await this.findSimilarExamples(archetypeDetection.primary, extractedKeywords)
      : [];

    // Step 6: Generate calculations (if applicable)
...
module.exports = new ProblemAnalyzer();
