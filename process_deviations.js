/**
 * Deviation Processing Script for guided_examples_v11.json
 * Processes all 224 problems with deviation detection and injection
 */

const fs = require('fs');
const path = require('path');

// Import the deviation utilities
// Since we can't use ES6 imports in Node without .mjs, we'll inline the logic
...
function detectDeviations(step, archetype, problem) {
  ...
  const combinedText = stepText + ' ' + problemText;
  ...
  for (const deviation of DEVIATION_DATABASE) {
    ...
    const patternMatch = checkPatternMatch(combinedText, deviation.patterns);
    const contextMatch = !deviation.contextPatterns ||
                        checkPatternMatch(combinedText, deviation.contextPatterns);
    const antiPatternMatch = deviation.antiPatterns &&
                            checkPatternMatch(combinedText, deviation.antiPatterns);
...
/**
 * Inject deviation alerts into problem solution steps
 */
function injectDeviationAlerts(problem) {
...
/**
 * Scan problem text for deviations
 */
function scanForDeviations(problemText, archetype) {
  const detected = [];
  ...
  for (const deviation of DEVIATION_DATABASE) {
    ...
    const patternMatch = checkPatternMatch(problemText, deviation.patterns);
    const contextMatch = !deviation.contextPatterns ||
                        checkPatternMatch(problemText, deviation.contextPatterns);
    const antiPatternMatch = deviation.antiPatterns &&
                            checkPatternMatch(problemText, deviation.antiPatterns);

    if (patternMatch && contextMatch && !antiPatternMatch) {
      detected.push(deviation.code);
    }
  }
...
async function processProblems() {
  const inputPath = path.join(__dirname, 'public', 'source-materials', 'guided_examples_v11.json');
  const outputPath = path.join(__dirname, 'public', 'source-materials', 'guided_examples_v11_enriched.json');
  const backupPath = path.join(__dirname, 'public', 'source-materials', 'guided_examples_v11_backup.json');
  const reportPath = path.join(__dirname, 'public', 'source-materials', 'deviation_processing_report.json');

  console.log('Starting deviation processing...');
  ...
  // Extract the worked_examples array
  const problems = data.worked_examples || data;
  if (!Array.isArray(problems)) {
    throw new Error('Expected worked_examples to be an array');
  }
  ...
  const enrichedProblems = problems.map((problem, index) => {
    if (index % 50 === 0) {
      console.log(`Processing problem ${index + 1}/${problems.length}...`);
    }

    stats.total_problems++;

    // Scan for deviations in problem text
    const archetype = problem.archetype || 'Unknown';
    const detectedDeviations = scanForDeviations(problem.problem_text || '', archetype);

    // Inject deviation alerts into solution steps
    const enrichedProblem = injectDeviationAlerts(problem);

    // Add detected_deviations array to problem
    enrichedProblem.detected_deviations = detectedDeviations;

    // Update statistics
    if (detectedDeviations.length > 0 || enrichedProblem.deviation_summary?.total_deviations > 0) {
      stats.problems_with_deviations++;
    }

    // Count deviation types
    detectedDeviations.forEach(devCode => {
      stats.deviation_breakdown[devCode] = (stats.deviation_breakdown[devCode] || 0) + 1;
    });

    // Track deviation combinations
    if (detectedDeviations.length > 1) {
      const combo = detectedDeviations.sort().join(' + ');
      stats.deviation_combinations[combo] = (stats.deviation_combinations[combo] || 0) + 1;
    }

    // Track time impact
    if (enrichedProblem.deviation_summary) {
      stats.total_time_impact += enrichedProblem.deviation_summary.total_time_impact_minutes;

      // Track severity
      Object.entries(enrichedProblem.deviation_summary.severity_distribution).forEach(([severity, count]) => {
        stats.severity_breakdown[severity] += count;
      });
    }

    // Track archetypes
    stats.archetype_breakdown[archetype] = (stats.archetype_breakdown[archetype] || 0) + 1;

    return enrichedProblem;
  });

  // Sort deviation breakdown by count
  const sortedBreakdown = Object.entries(stats.deviation_breakdown)
    .sort((a, b) => b[1] - a[1])
  ...
  stats.deviation_breakdown = sortedBreakdown;

  // Sort deviation combinations by count
  const sortedCombinations = Object.entries(stats.deviation_combinations)
    .sort((a, b) => b[1] - a[1])
  ...
  stats.deviation_combinations = sortedCombinations;
  stats.total_time_impact = Math.round(stats.total_time_impact * 10) / 10;

  // Prepare output with preserved structure
  const outputData = {
    ...data,
    metadata: {
      ...data.metadata,
      ...
      total_deviations_detected: stats.problems_with_deviations
    },
    worked_examples: enrichedProblems
  };

  // Write enriched data
  console.log('Writing enriched data to:', outputPath);
  fs.writeFileSync(outputPath, JSON.stringify(outputData, null, 2), 'utf8');

  // Write processing report
  console.log('Writing processing report to:', reportPath);
  const report = {
    timestamp: new Date().toISOString(),
    statistics: stats,
    deviation_database_count: DEVIATION_DATABASE.length,
    processing_notes: [
      'All existing fields preserved',
      'Added detected_deviations array to each problem',
      'Added deviation_alert fields to solution_steps where applicable',
      'Added deviation_summary to each problem',
  ...
  console.log('\n=== PROCESSING COMPLETE ===');
  console.log('Total problems processed:', stats.total_problems);
  console.log('Problems with deviations:', stats.problems_with_deviations);
  console.log('Total time impact (minutes):', stats.total_time_impact);
  console.log('\nTop deviations detected:');
  Object.entries(stats.deviation_breakdown).slice(0, 10).forEach(([code, count]) => {
    console.log(`  ${code}: ${count} problems`);
  });
  console.log('\nFull report saved to:', reportPath);
}

// Run the processing
processProblems().catch(error => {
